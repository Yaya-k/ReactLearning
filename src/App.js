    ...
    makeMessages() {
        ...
        while (result.length < 10) {
            result.push({
                id: result.length,
                show: true,
                txt: "Sed cautela niSed quid est quod in hac causa maxime homines admirentur et" +
                    ...
                    "mia in peiores haeserat plagas, ut narrabimus postea, aemulis consarcinantibus insidias graves apud " +
                    "Constantium, cetera medium principem sed siquid auribus eius huius modi quivis infudisset ignotus, acerbum " +
                    "et inplacabilem et in hoc causarum titulo dissimilem sui.",
    ...
    render() {
        return (
            <div className={this.state.isLight_mode ? "light-mode" : "dark-mode"}>
                ...
                <button
                    type="button"
                    disabled={!this.doesMessageDeleted }
                ...
                {this.state.messages.map(({txt,prt,user,show,id},index) => (
                    show ?
                        <Message
                            ...
                            id={id}
                            userName={user}
                            texte={this.state.hideMessagesArehidden && prt==="hidden" ? "this is a hidden message":txt}
                            porter={prt}
                        /> : ""
                ))}
            </div>
